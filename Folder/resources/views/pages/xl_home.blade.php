...
<head>
	...
	<title>Monitoring Progress XL</title>
</head>
<body>
	...
	<section id="content">
		...
		<main>
			...
			<!-- PO -->
			<a href="xl_add_po" class="btn-tambah">
				<b>+</b>
    ...
    <script>
        ...
        // add new ring
        $(document).on('click', '#add_new_ring_xl', function(e){
            var no_spk = $('#get_no_spk').val();
            window.location.href = '/xl_add_ring/' + no_spk;
        });
                            ...
                            $('#segment1').append(
                                '<tr id="fetch_data_segment">'+
                                    ...
                                    '<td class="action-segment">'+
                                        '<a href="xl_uploadfile.html">'+
                                        '<img src="img/dokumen.png">'+
                                        '</a>'+
                                        '<a href="xl_dokgambar.html">'+
		...
		// Add new segment
		$(document).on('click', '#add_new_segment_xl', function(e){
			var ring_id = $('#get_ring_id').val();	
			window.location.href = '/xl_add_segment/' + ring_id;
		});


		$(document).on('click', '.action-segment a[href="xl_dokgambar.html"]', function(e){
		    e.preventDefault();
		    var segment_id = $(this).closest('tr').find('input[type="hidden"]').last().val();
		    window.location.href = '/xl_dokgambar?segment_id=' + segment_id;
		});

		$(document).on('click', '.action-segment a[href="xl_uploadfile.html"]', function(e){
		    e.preventDefault();
		    var segment_id = $(this).closest('tr').find('input[type="hidden"]').last().val();
		    window.location.href = '/xl_document?segment_id=' + segment_id;
		});
